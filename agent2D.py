...
from network_models_2D import initialize_network_FCQN
...
class agent_2D:
    ...
    def reward_gen(self, length_of_path1, length_of_path2, goal_in_unknown_space=False, safe_travel=True):
        alpha = 0.003  # make training process more stable
        p_coeff = 0.4  # dictate how much to penalise path length

        if length_of_path1 == 0 or length_of_path2 == 0:  # no exploration done this iter
        ...
        reward = alpha * (gain_in_RE - p_coeff * ((length_of_path1 + length_of_path2) / 2)) - 0.7  # Extra -0.8 tagged on to hopefully reduce number of Goal points generated
